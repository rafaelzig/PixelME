...
import tsinghua.mediatech.rafaelzig.pixelme.camera.component.RecyclerViewClickPosition;
...
public class Camera2BasicFragment extends Fragment
		implements View.OnClickListener, FragmentCompat.OnRequestPermissionsResultCallback, RecyclerViewClickPosition
{

	public static final String IMAGE_FILE_LOCATION = "image_file_location";
	private final       String GALLERY_LOCATION    = "Image Gallery";
			...
			= new ImageReader.OnImageAvailableListener()
	{
		...
		{
			mBackgroundHandler.post(new ImageSaver(reader.acquireNextImage(), createImageFile(), uiHandler));
		}
	};
	...
	public void onViewCreated(final View view, Bundle savedInstanceState)
	{
		...
		recyclerView.setAdapter(new ImageAdapter(sortByLatest(galleryFolder), this));
	...
	private File createImageFile()
	{
		...
		try
		{
			image = File.createTempFile(imageFileName, ".jpg", galleryFolder);
		}
		catch (IOException e)
		{
			e.printStackTrace();
	...
	private File[] sortByLatest(File filesDir)
	{
		File[] sortedFiles = filesDir.listFiles();
	...
	{
		ImageAdapter imageAdapter = new ImageAdapter(sortByLatest(galleryFolder), this);
		recyclerView.swapAdapter(imageAdapter, false);
	}
	...
	private void captureStillPicture()
	{
		try
		{
			...
			mCaptureSession.capture(captureBuilder.build(), CaptureCallback, new Handler(activity.getMainLooper()));
		}
		catch (CameraAccessException e)
		{
			e.printStackTrace();
	...
	public void getRecyclerViewAdapterPosition(int position)
	{
		...
		sendFileAddressIntent.putExtra(IMAGE_FILE_LOCATION, sortByLatest(galleryFolder)[position].toString());
